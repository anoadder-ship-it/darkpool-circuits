...
function enc(v: bigint) {
  const nb = randomBytes(16);
  return { ct: Array.from(cipher.encrypt([v], nb)[0]) as any, n: new BN(deserializeLE(nb).toString()) };
}

async function waitCB(ms = 90000): Promise<string> {
  const dl = Date.now() + ms;
  while (Date.now() < dl) {
    await sleep(5000);
    const sigs = await conn.getSignaturesForAddress(PROGRAM_ID, { limit: 5 });
    for (const s of sigs.filter(s => !s.err)) {
      const td = await conn.getTransaction(s.signature, { maxSupportedTransactionVersion: 0, commitment: "confirmed" });
      const logs = td?.meta?.logMessages?.join("\n") || "";
      if (logs.includes("Callback")) {
        const name = logs.includes("MatchOrders") ? "MatchOrders" : logs.includes("PlaceOrder") ? "PlaceOrder" : logs.includes("CancelOrder") ? "CancelOrder" : "GetStats";
        return name;
      }
    }
  }
...
async function main() {
  ...
  await run("place_order (bid=100)", async () => {
    ...
    const t = await (prog.methods as any).placeOrder(off, b.ct, s.ct, ib.ct, pubKeyArr, b.n, s.n, ib.n)
    ...
    if (cb === "timeout") throw new Error("geen callback binnen 90s");
  ...
  await run("match_orders positief (buy=100, sell=95 → match)", async () => {
    ...
    const off = new BN(randomBytes(8), "hex");
    const t = await (prog.methods as any).matchOrders(off, b.ct, s.ct, pubKeyArr, b.n, s.n)
  ...
  await run("get_stats (buy_vol=150, sell_vol=100)", async () => {
    ...
    const off = new BN(randomBytes(8), "hex");
    const t = await (prog.methods as any).getStats(off, b.ct, s.ct, pubKeyArr, b.n, s.n)
