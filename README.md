...
10 concurrent `match_orders` transactions, 4 at a time, on Solana devnet -- covers both
callback-polling reliability and correct per-transaction result matching under load.

```
=== 10/10 PASS in 16.5s ===
```

Matching is done by polling the program's transaction signatures and identifying each
callback via its `computationAccount`, since concurrent WebSocket log subscriptions proved
unreliable on public devnet RPC endpoints.

---
## Security

All four darkpool programs' upgrade authority is held by a 2-of-3 Squads multisig, not a
single keypair. See [docs/MULTISIG_SECURITY.md](docs/MULTISIG_SECURITY.md) for the full
setup and member addresses.
