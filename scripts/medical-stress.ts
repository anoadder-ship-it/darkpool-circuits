...
// Gedeelde cache: elke tx-signature wordt maar 1x via RPC opgehaald.
...
// Checkt of een tx een specifiek account bevat (legacy EN v0 messages).
...
// Poll i.p.v. WebSocket. Match op het computationAccount van DEZE test
// (nonce kan intern +1 verschuiven voor output-encryptie -- niet betrouwbaar als match-sleutel).
async function waitForEvent(eventName: string, computationAccountBase58: string, timeoutMs = 90000): Promise<bigint[] | null> {
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    for (const s of sigs) {
      ...
      if (!tx) continue;
      if (!txBevatAccount(tx, computationAccountBase58)) continue; // hoort bij ANDERE test
...
async function oneTest(i: number): Promise<boolean> {
  const matchCase = i % 2 === 0;
  const queryDisease = matchCase ? BigInt(340) : BigInt(174);
  const enc = encTogether([
    BigInt(340), BigInt(5000), BigInt(580), BigInt(40), BigInt(2),
    queryDisease, BigInt(1000), BigInt(500), BigInt(700), BigInt(2),
  ]);
  ...
  try {
    const t = await (prog.methods as any).matchDataset(
      off, enc.cts[0], enc.cts[1], enc.cts[2], enc.cts[3], enc.cts[4],
  ...
  const result = await waitForEvent("datasetMatchedEvent", compAccBase58);
  if (!result) { console.log("TIMEOUT #" + i + " (geen callback binnen 90s)"); return false; }
  const compatible = result[0] === BigInt(1);
  const expected = matchCase;
  const ok = compatible === expected;
  console.log((ok ? "PASS" : "FAIL") + " #" + i + "  case=" + (matchCase ? "MATCH" : "GEEN MATCH") +
              "  compatible=" + compatible + " score=" + result[1] + "/100");
  return ok;
}
...
async function main() {
  ...
  console.log("=== Medical Darkpool Stress Test: " + N_TESTS + " transacties, " + CONCURRENCY + " tegelijk ===");
  const start = Date.now();
