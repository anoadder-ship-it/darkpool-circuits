...
async function oneTest(i: number): Promise<boolean> {
  ...
  const queryChipType = matchCase ? BigInt(1001) : BigInt(1002); // H100 vs H200
  const enc = encTogether([
    BigInt(1001), BigInt(10), BigInt(1), BigInt(3500000), BigInt(14), BigInt(1), BigInt(1), // aanbod: H100
    queryChipType, BigInt(5), BigInt(3), BigInt(4000000), BigInt(30), BigInt(1), BigInt(1),  // vraag
  ]);
  ...
  const result = await waitForEvent("chipMatchedEvent", compAccBase58);
  ...
  console.log((ok ? "PASS" : "FAIL") + " #" + i + "  case=" + (matchCase ? "MATCH" : "GEEN MATCH") +
              "  matched=" + matched + " score=" + result[1] + "/98");
  return ok;
}
...
async function main() {
  ...
  console.log("=== Chip Marketplace Darkpool Stress Test: " + N_TESTS + " transacties, " + CONCURRENCY + " tegelijk ===");
  const start = Date.now();
